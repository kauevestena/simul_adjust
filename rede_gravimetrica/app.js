...
const app = {
    ...
    updateApriori() {
        ...
        // Ensure std represents the selected stochastic model for statistical tests
        const modelSelection = document.querySelector('input[name="stochastic_model"]:checked');
        const stochModel = modelSelection ? modelSelection.value : 'variance';

        this.observations.forEach(obs => {
            if (stochModel === 'variance') {
                obs.std = Math.sqrt(obs.variance);
            } else if (stochModel === 'heuristic') {
                obs.std = Math.sqrt(1.0 / obs.heuristic_weight);
            } else {
                obs.std = 1.0;
            }
        });
    ...
    generateNetwork() {
        ...
        // Carrega os pontos da defaultNetworkData
        this.points = [];
        this.observations = [];

        if (typeof defaultNetworkData !== 'undefined') {
            this.points = defaultNetworkData.nodes.map(p => ({
                id: p.id,
                x: p.lng,
                y: p.lat,
                fixed: p.fixed,
            ...
            defaultNetworkData.links.forEach((link, idx) => {
                const pFrom = this.points.find(p => p.id === link.source);
                const pTo = this.points.find(p => p.id === link.target);
    ...
    validateGravimetricNetwork() {
        const errors = [];
        const pointIds = new Set(this.points.map(p => p.id));
        if (!this.points.some(p => p.fixed)) {
            errors.push('A rede precisa de ao menos um ponto fixo para definir o datum gravimétrico.');
        }
    ...
    injectOutlier() {
        ...
        list.innerHTML = '';
        this.observations.forEach((obs, idx) => {
            const valDisplay = obs.val.toFixed(3) + ' mGal';
    ...
    confirmBlunder() {
        ...
        selected.forEach(idx => {
            ...
            obs.hasError = true;
            const errDisplay = magnitude.toFixed(3) + ' mGal';
    ...
    runAdjustment() {
        ...
        const validationErrors = this.validateGravimetricNetwork();
        if (validationErrors.length > 0) {
            alert(`Rede gravimétrica inválida:\n${validationErrors.join('\n')}`);
            return;
        }

        // Re-sample Gaussian noise on every run; if sigma=0 no noise is added
        const sigLin = parseFloat(document.getElementById('simSigLin').value); // mGal
        this.observations.forEach(obs => {
        ...
        const stochModel = document.querySelector('input[name="stochastic_model"]:checked').value;
        this.observations.forEach((o, i) => {
            if (stochModel === 'variance') {
                P[i][i] = 1.0 / o.variance;
            } else if (stochModel === 'heuristic') {
                P[i][i] = o.heuristic_weight;
            } else if (stochModel === 'unit') {
                P[i][i] = 1.0;
            }
        });
        ...
        // Linear system: A is constant for gravimetry networks
        ...
        const obsData = this.observations.map((o, i) => {
            ...
            // External reliability: gravity displacement caused by undetected MDB
            const ext = reliable
    ...
    drawNetwork() {
        ...
        this.points.forEach(pt => {
            ...
            hLabel.innerText = `g: ${pt.H.toFixed(3)}`;
            ...
            if (this.adjResults && !pt.fixed) {
                ...
                if (pResult) {
                    ...
                    const barH = stdH * ellipseExag; // std em mGal * exage
    ...
    updateUI_Results() {
        ...
        res.obsData.forEach(r => {
            ...
            const obsDisplay  = r.obs.val.toFixed(3) + ' mGal';
            const vDisplay    = r.v.toFixed(3) + ' mGal';
            const svDisplay   = `${r.sigma_v.toFixed(3)} mGal <span class="text-stone-400 text-[10px]">(${(res.sigma0 * r.sigma_v).toFixed(3)})</span>`;
        ...
        res.obsData.forEach(r => {
            ...
            const mdbDisplay = r.mdb == null ? '--' : r.mdb.toFixed(3) + ' mGal';
        ...
        res.stationResults.forEach(sr => {
            const tr = document.createElement('tr');
            const sigH_pri = Math.sqrt(sr.QxxBlock); // mGal (a-priori)
            const sigH_pos = res.sigma0 * sigH_pri; // mGal (a-posteriori)
            const extMm = sr.maxExtMag; // mGal
            tr.innerHTML = `
    ...
    renderMatrixTab() {
        ...
        const explanations = {
            ...
            'X': '<strong>Vetor de Solução (dx):</strong> Correções estimadas por mínimos quadrados (dx = N<sup>-1</sup>U). Na gravimetria, o modelo é linear, então a solução é obtida em uma etapa após montar A, P e L.',
            ...
            'N': '<strong>Matriz das Equações Normais (N):</strong> Equacionada por N = A<sup>T</sup> P A, condensa o modelo estocástico (peso) e geométrico da rede numa matriz simétrica. Ela só é positiva definida quando há datum gravimétrico e cada componente desconhecida está conectada a ponto fixo. <em>Nota: valores altos são normais porque os pesos são inversos das variâncias.</em>',
    ...
    renderObsTable() {
        ...
        if (this.observations.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="text-center text-stone-400 py-4">Nenhuma observação.</td></tr>';
            return;
        }
        ...
        this.observations.forEach((obs, idx) => {
            ...
            tr.innerHTML = `
                <td>${obs.from}</td>
                <td>${obs.to}</td>
                <td>${obs.val.toFixed(3)}</td>
                <td>${obs.std.toFixed(3)}</td>
                <td></td>
            `;
    ...
    runMonteCarlo() {
        ...
        setTimeout(() => {
            ...
            // A matrix is constant for gravimetry
            const A = this.observations.map(() => Array(nu).fill(0));
            this.observations.forEach((o, i) => {
                const pFrom = this.points.find(p => p.id === o.from);
                const pTo = this.points.find(p => p.id === o.to);
